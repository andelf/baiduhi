...
start_link() ->
    LogBase =
        case baiduhi_config:get_env(logger_logbasename) of
            {ok, LogBase1} -> LogBase1;
            none ->
                case application:get_application() of
                    {ok, Name} when is_atom(Name) ->
                        AppName = atom_to_list(Name),
                        case baiduhi_config:get_env(logger_logdir) of
                            {ok, LogDir} ->
...
init([Basename]) ->
    ...
    {ok, Status} = baiduhi_config:get_env(logger_status),
...
handle_info(check_logfile_size, State) ->
    ...
    case baiduhi_config:get_env(max_logfile_size) of
	{ok, 0} ->
	    {noreply, State};
	{ok, Size} ->
...
create_filename_time_suffix() ->
    ...
    DateTime = logger:sec_to_date(Megasec * 1000000 + Sec),
...
get_ts({Megasec, Sec, USec}) ->
    ...
    DateTime = sec_to_date(Megasec * 1000000 + Sec),
